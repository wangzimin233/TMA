...
type DepositSelectionDrawersProps = {
  coinOpen: boolean;
  networkOpen: boolean;
  selectedCoin: DepositCoin | null;
  onCoinOpenChange: (open: boolean) => void;
  onNetworkOpenChange: (open: boolean) => void;
  onSelectCoin: (coin: DepositCoin) => void;
  onSelectNetwork: (network: DepositNetwork) => void;
};

export function DepositSelectionDrawers({
  coinOpen,
  networkOpen,
  selectedCoin,
  onCoinOpenChange,
  ...
  onSelectCoin,
  onSelectNetwork,
}: DepositSelectionDrawersProps) {
  const [keyword, setKeyword] = useState('');
  const { data: coins = [], isError, isLoading, refetch } = useDepositCoins(coinOpen);
  ...
  return (
    <>
      <Drawer open={coinOpen} onOpenChange={onCoinOpenChange} direction="bottom">
        <DrawerContent className="mx-auto max-h-[86vh] w-full max-w-[430px] border-line bg-base pb-[env(safe-area-inset-bottom)] text-ink">
          <DrawerHeader className="px-4 pb-2 pt-4 text-left">
            <DrawerTitle className="text-[1.05rem] font-semibold text-ink">选择资产</DrawerTitle>
            <DrawerDescription className="sr-only">搜索并选择要充值的资产</DrawerDescription>
          </DrawerHeader>
      ...
      <Drawer open={networkOpen} onOpenChange={onNetworkOpenChange} direction="bottom">
        <DrawerContent className="mx-auto max-h-[86vh] w-full max-w-[430px] border-line bg-base pb-[env(safe-area-inset-bottom)] text-ink">
          <DrawerHeader className="px-4 pb-2 pt-4 text-left">
            <DrawerTitle className="text-[1.05rem] font-semibold text-ink">选择网络</DrawerTitle>
            <DrawerDescription className="text-left text-[0.74rem] text-muted-foreground">
              {selectedCoin ? `充值 ${selectedCoin.coinCode} 前请确认链网络` : '请选择资产后再选择网络'}
            </DrawerDescription>
          </DrawerHeader>
            ...
            {selectedCoin?.networks.length ? (
              selectedCoin.networks.map((network) => (
                ...
                >
                  ...
                  <span className="block text-[0.76rem] leading-6 text-muted-foreground">
                    最低充值 &gt; {formatNumber(network.minDepositAmount)} {selectedCoin.coinCode}
                  </span>
                  <span className="block text-[0.76rem] leading-6 text-muted-foreground">
                    预计到账时间 ≈ {getArrivalEstimate(network.confirmCount)}
                  </span>
                </button>
              ))
            ) : (
...
function CoinAvatar({ coin }: { coin: DepositCoin }) {
  ...
  return (
    <span className="grid size-8 shrink-0 place-items-center rounded-full bg-brand/15 text-[0.72rem] font-semibold text-brand">
...
function getArrivalEstimate(confirmCount: number) {
  if (confirmCount <= 1) return '1 分钟';
  if (confirmCount <= 6) return '2 分钟';
  if (confirmCount <= 12) return '3 分钟';
  return '5 分钟';
}
